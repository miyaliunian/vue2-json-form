...
;(function () {
  ...
  const matchNodeName = (name) => (node) =>
    node && node.nodeName.toLowerCase() === name
  ...
  const listState = (editor, listName, activate) => {
    const nodeChangeHandler = (e) => {
      const inList = findUntil(e.parents, isListNode, isTableCellNode)
        .filter((list) => list.nodeName === listName && !isCustomList(list))
        .isSome()
      activate(inList)
    }
